...
architecture Behavioral of HB19 is

    constant c_taps: natural:= 19;
    constant c_multipliers: natural := 11;
    constant c_MUL1shiftbits: integer := -11; -- increase this if overflow, decrease if low resolution

    type t_BUFF0 is array(0 to c_taps - 1) of signed(c_HB19bits-1 downto 0);
    signal BUFF0: t_BUFF0 := (others => (others => '0')); -- input buffer

    type t_ADD2data is array(0 to 1) of signed(c_HB19bits-1 downto 0); -- to easily switch input data of adder
    signal ADD2data: t_ADD2data := (others => (others => '0')); -- 0 => 0, 1 => output(input1)
    signal ADD2datain2: signed(c_HB19bits-1 downto 0) := (others => '0');

    type t_MUL1data0 is array(0 to c_multipliers-1) of signed((2*c_HB19bits)-1 downto 0);
    type t_MUL1data is array(0 to c_multipliers-1) of signed(c_HB19bits-1 downto 0);
...
begin 
    ...
    begin
        if rising_edge(i_clk_MUL1) then
            -- multiplications
            MUL1data0(0) <= BUFF0(0) * c_HB19coefs(0);
            MUL1data0(1) <= BUFF0(2) * c_HB19coefs(1);
            MUL1data0(2) <= BUFF0(4) * c_HB19coefs(2);
            MUL1data0(3) <= BUFF0(6) * c_HB19coefs(3);
            MUL1data0(4) <= BUFF0(8) * c_HB19coefs(4);
            MUL1data0(5) <= BUFF0(9) * c_HB19coefs(5);
            MUL1data0(6) <= BUFF0(10) * c_HB19coefs(4);
            MUL1data0(7) <= BUFF0(12) * c_HB19coefs(3);
            MUL1data0(8) <= BUFF0(14) * c_HB19coefs(2);
            MUL1data0(9) <= BUFF0(16) * c_HB19coefs(1);
            MUL1data0(10) <= BUFF0(18) * c_HB19coefs(0);
